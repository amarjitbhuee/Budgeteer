...
class AprStatementBalance extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
            transactionsIncome: [],
    ...
    getExpense = () => {
        let url = "http://localhost:3001/transactions/expenseApril"; 
        axios.get(url)
            .then(response => this.setState({ transactionsExpense: response.data}));
    };

    getSavings = () => {
        let url = "http://localhost:3001/transactions/savingsApril"; 
        axios.get(url)
...
export default AprStatementBalance; 