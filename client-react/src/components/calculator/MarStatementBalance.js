...
class MarStatementBalance extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
            transactionsIncome: [],
    ...
    getIncome = () => {
        let url = "http://localhost:3001/transactions/incomeMarch";
        axios.get(url)
    ...
    getExpense = () => {
        let url = "http://localhost:3001/transactions/expenseMarch"; 
        axios.get(url)
            .then(response => this.setState({ transactionsExpense: response.data}));
    };

    getSavings = () => {
        let url = "http://localhost:3001/transactions/savingsMarch"; 
        axios.get(url)
...
export default MarStatementBalance; 