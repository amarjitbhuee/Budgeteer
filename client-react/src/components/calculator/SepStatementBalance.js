...
class SepStatementBalance extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
            transactionsIncome: [],
    ...
    getIncome = () => {
        let url = "http://localhost:3001/transactions/incomeSeptember";
        axios.get(url)
    ...
    getExpense = () => {
        let url = "http://localhost:3001/transactions/expenseSeptember"; 
        axios.get(url)
            .then(response => this.setState({ transactionsExpense: response.data}));
    };

    getSavings = () => {
        let url = "http://localhost:3001/transactions/savingsSeptember"; 
        axios.get(url)
...
export default SepStatementBalance; 