...
class NovStatementBalance extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
            transactionsIncome: [],
    ...
    getIncome = () => {
        let url = "http://localhost:3001/transactions/incomeNovember";
        axios.get(url)
    ...
    getExpense = () => {
        let url = "http://localhost:3001/transactions/expenseNovember"; 
        axios.get(url)
            .then(response => this.setState({ transactionsExpense: response.data}));
    };

    getSavings = () => {
        let url = "http://localhost:3001/transactions/savingsNovember"; 
        axios.get(url)
...
export default NovStatementBalance; 