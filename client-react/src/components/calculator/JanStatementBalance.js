...
class JanStatementBalance extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
            transactionsIncome: [],
    ...
    getIncome = () => {
        let url = "http://localhost:3001/transactions/incomeJanuary";
        axios.get(url)
    ...
    getExpense = () => {
        let url = "http://localhost:3001/transactions/expenseJanuary"; 
        axios.get(url)
            .then(response => this.setState({ transactionsExpense: response.data}));
    };

    getSavings = () => {
        let url = "http://localhost:3001/transactions/savingsJanuary"; 
        axios.get(url)
...
export default JanStatementBalance; 